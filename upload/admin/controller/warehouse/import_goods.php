...
class ControllerWarehouseImportGoods extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				'barcode'    => $this->barcoderender( $result['model'] ),
		...
		$this->response->setOutput($this->load->view('warehouse/import_goods', $data));
	}
	...
	public function barcoderender($code)
	{
		$generator = new \Picqer\Barcode\BarcodeGeneratorPNG();
		

		$path = DIR_IMAGE .'product_barcode/';

		$barcode_path = $path . $code . '.png';


		if (file_get_contents($barcode_path) === true) {
			return $barcode_path;
		} else {
			if (!file_exists($path)) {
				mkdir($path, 0700);
			...
			file_put_contents($barcode_path,  $generator->getBarcode($code, $generator::TYPE_CODE_128));
			return $barcode_path;
		}
